...
export async function POST(
  ...
  try {
    ...
    const sandbox = await sandboxService.startSandbox(replId, user.userId);
    return NextResponse.json({
      success: true,
      sandbox,
      message: `Sandbox for project ${replId} is running`,
    });
  } catch (err: any) {
    console.error("[API] Start sandbox error:", err);
